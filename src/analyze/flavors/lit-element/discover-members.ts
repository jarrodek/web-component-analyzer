    ...
    return [
      {
        ...
        type: lazy(() => {
          ...
          return inJavascriptFile && typeof litConfig.type === 'object' && litConfig.type.kind === 'ANY'
            ? litConfig.type
            : propType
        }),
        node,
...
function inPolymerFlavorContext(context: AnalyzerDeclarationVisitContext): boolean {
  ...
  if (
    context
      .getDeclaration()
      .heritageClauses.some((c) => ['PolymerElement', 'Polymer.Element'].includes(c.identifier.getText()))
  ) {
    result = true
  }
  ...
  if (returnStatement.expression != null && ts.isObjectLiteralExpression(returnStatement.expression)) {
    ...
    for (const propNode of returnStatement.expression.properties) {
      ...
      memberResults.push({
        ...
        type: lazy(() => {
          return (
            (jsDoc && getJsDocType(jsDoc, context)) ||
            (typeof litConfig.type === 'object' && litConfig.type) || { kind: 'ANY' }
          )
        }),
        propName: propName,
