  ...
  if (ts.isGetAccessor(node) && hasModifier(node, ts.SyntaxKind.StaticKeyword, ts)) {
    if (node.name.getText() === 'observedAttributes' && node.body != null) {
      ...
      const arrayLiteralExpression =
        (node.body.statements.find((statement) => ts.isReturnStatement(statement)) as ReturnStatement | undefined)
          ?.expression ?? node.body.statements.find((statement) => ts.isArrayLiteralExpression(statement))
      ...
      return [
        {
          ...
          propName: name.text,
          type: lazy(() =>
            parameter == null ? context.checker.getTypeAtLocation(node) : context.checker.getTypeAtLocation(parameter)
          ),
          visibility: getMemberVisibilityFromNode(node, ts),
          modifiers: getModifiersFromNode(node, ts),
        },
