...
function getExportsDocFromDeclaration(
  ...
  const superclassRef =
    superclassHeritage == null ? undefined : getReferenceFromHeritageClause(superclassHeritage, context)
...
function getEventDocsFromDeclaration(declaration: ComponentDeclaration, context: TransformerContext): EventDoc[] {
  return filterVisibility(context.config.visibility, declaration.events).map((event) => {
    ...
    const customEventDetailType =
      typeName === 'CustomEvent' && simpleType.kind === 'GENERIC_ARGUMENTS' ? simpleType.typeArguments[0] : undefined
