...
`,
  ...
  const filteredResults = results.filter(
    (result) =>
      result.componentDefinitions.length > 0 || result.globalFeatures != null || (result.declarations?.length || 0) > 0
  )
 ...
 * @param config
 */
export async function analyzeAndTransformGlobs(
  inputGlobs: string | string[],
  config: AnalyzerCliConfig
): Promise<string> {
 ...
 * @param config
 */
async function distributeResultsIntoFiles(
  results: AnalyzerResult[],
  config: AnalyzerCliConfig
): Promise<Map<string | null, AnalyzerResult[]>> {
  ...
  // Output all results into multiple files
    ...
    for (const result of results) {
      ...
      const filename = relative(
        process.cwd(),
        basename(result.sourceFile.fileName, extname(result.sourceFile.fileName))
      )
