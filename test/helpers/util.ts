...
export function assertHasMembers(
  ...
  for (const expectedMember of expectedMembers) {
    const matchedMembers = actualMembers.filter(
      (member) =>
        expectedMember.kind === member.kind &&
        expectedMember.attrName === member.attrName &&
        expectedMember.propName === member.propName
    )
    ...
    'attrName' in expectedMember &&
      assert.strictEqual(actualMember.attrName, expectedMember.attrName, `Attribute names are not the same`)
    'propName' in expectedMember &&
      assert.strictEqual(actualMember.propName, expectedMember.propName, `Property names are not the same`)
    'default' in expectedMember &&
      assert.deepEqual(actualMember.default, expectedMember.default, `Default value for ${name} doesn't match`)
    'visibility' in expectedMember &&
      assert.strictEqual(actualMember.visibility, expectedMember.visibility, `Visibility for ${name} doesn't match`)
    'reflect' in expectedMember &&
      assert.strictEqual(actualMember.reflect, expectedMember.reflect, `Reflect for ${name} doesn't match`)
    'required' in expectedMember &&
      assert.strictEqual(actualMember.required, expectedMember.required, `Required for ${name} doesn't match`)
    'deprecated' in expectedMember &&
      assert.strictEqual(actualMember.deprecated, expectedMember.deprecated, `Deprecated for ${name} doesn't match`)
    'typeHint' in expectedMember &&
      assert.strictEqual(actualMember.typeHint, expectedMember.typeHint, `TypeHint for ${name} doesn't match`)
    'jsDoc' in expectedMember &&
      assert.strictEqual(
        actualMember?.jsDoc?.description,
        expectedMember?.jsDoc?.description,
        `JSDoc for ${name} doesn't match`
      )
...
export function getComponentProp(members: ComponentMember[], propName: string): ComponentMemberProperty | undefined {
  return members.find((member) => member.kind === 'property' && member.propName === propName) as
